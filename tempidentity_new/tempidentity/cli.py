...
from tempidentity.core import (
    load_config, save_config, get_history,
    create_temp_email, check_email_messages, get_email_message_content,
    get_sms_services, create_temp_sms, wait_for_sms_code, cancel_sms_number,
    get_available_providers, configure_provider
)
...
def create_temp_email_ui():
    ...
    if wait_for_msgs:
        ...
        with spinner(f"Waiting for messages (timeout: {wait_time}s)") as sp:
            new_messages = check_email_messages(service_name, email, password, True, wait_time)
            if new_messages:
            ...
            for idx, message in enumerate(new_messages):
                ...
                if HAS_RICH:
                    ...
                    if 'text' not in message and 'html' not in message and 'id' in message:
                        detailed_message = get_email_message_content(service_name, email, password, message['id'])
                        if detailed_message:
    ...
    try:
        ...
        if 0 <= choice_idx < len(history):
            ...
            if confirm("\nCheck for new messages?", default=False):
                ...
                wait_time = prompt("How long to wait for messages (seconds)?", default="60")
                try:
                    wait_time = int(wait_time)
                except:
                    wait_time = 60
                ...
                with spinner("Checking for messages") as sp:
                    messages = check_email_messages(service_name, email, password, wait, wait_time)
                    if messages:
                ...
                if messages:
                    if HAS_RICH:
                        ...
                        for idx, message in enumerate(messages):
                            print(f"{idx + 1}. {message.get('subject', 'No Subject')} - From: {message.get('from', {}).get('address', 'Unknown')}")
                    ...
                    try:
                        ...
                        if msg_idx >= 0 and msg_idx < len(messages):
                            ...
                            if HAS_RICH:
                                console.print(Panel(f"Subject: {message.get('subject', 'No Subject')}", title="Message Details"))
                                console.print(f"From: {message.get('from', {}).get('address', 'Unknown')}")
                                console.print(f"Date: {message.get('createdAt', 'Unknown')}")
...
def configure_providers_ui(provider_type: str):
    ...
    if HAS_RICH:
        ...
        for idx, (name, provider_class) in enumerate(providers.items()):
            status = f" {Fore.GREEN}(Current){Style.RESET_ALL}" if name == current_provider else ""
            print(f"{idx + 1}. {provider_class.display_name} ({name}): {provider_class.description}{status}")
    ...
    try:
        ...
        if HAS_RICH:
            console.print(f"[bold cyan]Configuring {provider_class.display_name}[/bold cyan]")
        else:
            print(f"{Fore.CYAN}Configuring {provider_class.display_name}{Style.RESET_ALL}")
        ...
        make_preferred = confirm(f"Set as preferred {provider_type} provider?", default=True)
        ...
        if setup_fields:
            ...
            for field in setup_fields:
                ...
                current_value = config.get('providers', {}).get(provider_name, {}).get(name, '')
                ...
                # Get new value
                value = prompt(f"Enter value (leave empty to keep current)", password=(field_type == 'password'))
...
def general_settings_ui():
    ...
    if HAS_RICH:
        ...
        table.add_row("1. Default Wait Time", f"{config.get('default_wait_time', 120)} seconds")
        table.add_row("2. Save History", "Enabled" if config.get('save_history', True) else "Disabled")
        table.add_row("3. History Limit", f"{config.get('history_limit', 20)} items")
        table.add_row("0. Back", "")
        
        console.print(table)
        
        choice = Prompt.ask("Select an option to change", choices=["0", "1", "2", "3"], default="0")
    else:
        print(f"{Fore.CYAN}General Settings:{Style.RESET_ALL}")
        print(f"1. Default Wait Time: {config.get('default_wait_time', 120)} seconds")
        print(f"2. Save History: {'Enabled' if config.get('save_history', True) else 'Disabled'}")
