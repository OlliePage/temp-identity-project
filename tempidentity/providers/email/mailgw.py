...
class MailGwProvider(EmailProvider):
    ...
    def create_email(self) -> Tuple[bool, str, str]:
        ...
        response = requests.post(
            f"{self.base_url}/accounts", headers=headers, data=json.dumps(payload)
        )
    ...
    def _login(self) -> bool:
        ...
        response = requests.post(
            f"{self.base_url}/token", headers=headers, data=json.dumps(payload)
        )
    ...
    def wait_for_messages(
        self, timeout: int = 60, check_interval: int = 5
    ) -> List[Dict]:
