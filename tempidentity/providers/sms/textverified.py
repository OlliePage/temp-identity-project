...
class TextVerifiedProvider(SMSProvider):
    ...
    def create_number(self, service_name: str) -> Tuple[bool, str]:
        ...
        payload = {"id": service_name}

        response = requests.post(
            f"{self.base_url}/Verifications", headers=headers, data=json.dumps(payload)
        )
    ...
    def check_sms(self) -> Optional[str]:
        ...
        response = requests.get(
            f"{self.base_url}/Verifications/{self.verification_id}", headers=headers
        )
